...
def _clamp_rul_minutes(model: str, val_min: float) -> float:
    """
    Clamp RUL predictions to reasonable bounds.
    
    CRITICAL FIX: Removed min_rul_hours clamp for base model.
    The original 83.33 hours (5000 min) minimum was causing all predictions
    to be clamped to 5000 min even when actual RUL was much higher.
    
    Now: Only apply maximum bounds for Stage-2A, no artificial minimum.
    """
    global base_config, stage2a_config
    v = float(val_min)

    # NOTE: Removed min_rul_hours clamp - it was causing incorrect predictions
    # The base model should be able to predict full range of RUL values
